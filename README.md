...
>
> ⚠️ **构建方式说明（v1.11.2 起）**：本版本起改为 **GitHub Actions 云端自动构建**，CI 使用与本机一致的 Android 调试签名（debug keystore），因此从你本机安装的旧版本（v1.11.1 及更早）升级时**可直接覆盖安装、无需卸载**。极少数曾装过早期 upload-key 测试包（v1.11.2 首发版）的情况，若升级报错则卸载后重装即可。
